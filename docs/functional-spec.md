...
### 2.1 시작 화면
- **기능**: 앱 시작 시 스플래시 화면 표시 후 메인 화면으로 자동 진입
- **화면**: SplashScreen → LoginScreen (타이틀 화면)
- **상태**: 구현 완료 (인증 없이 바로 진입)
...
- 현재 단계 이름
- 현재 단계 번호 / 전체 단계 수
- 남은 시간 (HH:MM:SS) — 1초마다 갱신
- 전체 진행률 (Progress Bar)

**완료 상태**:
- 초록색 박스 + 체크 아이콘 + "모든 단계 완료!" + "수고하셨습니다" 표시
- 프로그레스 바 초록색으로 꽉 찬 상태
...
- **서비스 시작**: 타이머 시작 시에만 서비스 실행 (앱 시작 시 자동 실행 안 함)
- **서비스 종료**: 모든 타이머 완료 시 서비스 자동 종료
- **알림**: 다음 단계로 전환될 때 푸시 알림 발송
- **완료 알림**: 모든 단계 완료 시 완료 알림 발송
- **UI 업데이트**: 타이머 화면 활성화 시에만 1초마다 데이터 전송 (배터리 최적화)
...
#### 2.7.1 알림 설정
- **타이머 알림 토글**: 단계 전환 및 완료 알림 켜기/끄기
- **기본값**: 켜짐

#### 2.7.2 앱 정보
- 앱 이름, 버전 정보, 회사(JJST Software) 표시
- 앱 제목 탭 시 시작 화면(LoginScreen)으로 이동

#### 2.7.3 피드백 / 문의
- 이메일로 문의 및 버그 리포트 전송

#### 2.7.4 Special Thanks
- 건강 빵집 리빙 브레드 스마트스토어 링크
...
### 3.3 타이머 업데이트 플로우
```
1. Background Service가 매초 타이머 상태 계산
2. TimerScreen이 활성화(subscribe)된 경우에만
   'update' 이벤트로 UI에 데이터 전송
   - activeTimers 리스트
   - 각 타이머의 현재 상태
3. TimerScreen이 이벤트 수신
4. setState로 UI 갱신 (1초마다)
5. 단계 전환 시 → 다음 단계 시작 알림 발송
6. 전체 완료 시 → 완료 알림 발송 후 DB 삭제
```
